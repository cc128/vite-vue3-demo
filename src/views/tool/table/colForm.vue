...
<template>
    ...
    <el-col v-for="(item, i) in _this.formQuery" :key="i"
        :span="(colType == 'form') ? (item.formCol || 12) : (item.queryCol || 4)">
        <el-form-item :label="item.label" :prop="item.value"
            :label-width="item.labelWidth || (colType == 'form' ? '80px' : '')">
            <slot :row="colType + '-' + item.value" :form="formParams" :placeholder="item.label">
...
let props = defineProps({
    ...
    colType: { type: String, default: "query" },
